...
bootstrap_postgres_roles() {
	...
	*) echo "warning: DATABASE_URL host '$app_host' is not local; these scripts only bootstrap the local Postgres cluster" >&2 ;;
	...
	# Zero's change-streamer runs CREATE PUBLICATION ... FOR TABLES IN SCHEMA, which
	# requires SUPERUSER on the ZERO_UPSTREAM_DB role. Docker Compose's POSTGRES_USER
	# is a superuser by default; match that for the upstream role. Keep the app role
	# least-privilege when DATABASE_URL and ZERO_UPSTREAM_DB use different users.
