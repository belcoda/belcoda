...
export const dialingCodesArray: {
	name: string;
	dial_code: string;
	code: CountryCode;
}[] = [
...
export function getPhoneNumberExample(
...
export function getCountryCodeFromPhoneNumber(phoneNumber: string): CountryCode {
	...
	if (!isValidCountryCode(phone.regionCode)) {
		throw new Error(`Invalid country code: ${phone.regionCode}`);
	}
...
export function renderLocalPhoneNumber(
	...
	if (!phone.valid) {
		return phoneNumber; // Return the original number if invalid and strict is false
	}
