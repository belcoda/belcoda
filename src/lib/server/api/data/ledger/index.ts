...
import { createLedgerEntrySchema } from '$lib/schema/ledger';
...
export async function _createLedgerEntry({
	...
	try {
		const parsed = await parse(createLedgerEntrySchema, args);
		...
		const [updated] = await tx.dbTransaction.wrappedTransaction
			...
			.returning();
		if (!updated) {
			throw new Error('Failed to update organization balance');
		}
	} catch (error) {
		log.error({ error }, 'Failed to create ledger entry');
		throw error;
	}
}
