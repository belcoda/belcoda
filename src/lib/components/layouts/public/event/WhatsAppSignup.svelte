<script lang="ts">
	...
	import type { PublicEventLayout } from '$lib/utils/event/analytics';
	import { trackEventWhatsAppHandoffOpened } from '$lib/utils/event/analytics';
		...
		{#if qrCodeDataUrl}
			<div class="mb-6 border-t border-gray-200 pt-6">
				...
				<div class="flex flex-col items-center gap-4">
					<img src={qrCodeDataUrl} alt={t`WhatsApp Signup QR Code`} class="rounded-lg border" />
					<p class="text-xs text-gray-500">{t`Scan with your phone to sign up via WhatsApp`}</p>
				</div>
			</div>
		{/if}
	</div>
