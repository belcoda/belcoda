...
<div class="mt-3 grid w-full max-w-md gap-4">
	<div class="text-sm text-muted-foreground">Scan this QR code to mark attendance via WhatsApp</div>
	...
	>
		{#if copied}<CheckIcon /> Copied!
		{:else}<CopyIcon />Copy WhatsApp checkin link{/if}
	</Button>
</div>
