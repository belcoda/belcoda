...
{#if emailMessage.data}
	<div class="w-full px-4 py-2 text-center text-sm text-gray-400">
		...
		<div class="flex items-center justify-center gap-1">
			Email sent:
			<a class="flex items-center gap-1" href={`/communications/email/sent/${emailMessage.data.id}`}>
				<Mail class="size-3" />
				<div class="font-medium underline hover:text-gray-500">{emailMessage.data.subject || '(no subject)'}</div>
			</a>
		</div>
	</div>
{/if}
