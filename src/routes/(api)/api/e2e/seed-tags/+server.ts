...
export const POST: RequestHandler = async ({ locals, request }) => {
	...
	const now = Date.now();
	const rows: (typeof schema.tag.$inferInsert)[] = Array.from({ length: count }, (_, index) => {
		const createdAt = new Date(now - (count - index - 1) * 1000);
		return {
			id: crypto.randomUUID(),
			organizationId,
			name: `E2E pagination tag ${runId}-${index + 1}`,
			active: true,
			createdAt,
			updatedAt: createdAt
		};
	...
	const inserted = await drizzle.insert(schema.tag).values(rows).returning({ id: schema.tag.id });
	...
	return json({
		...
		organizationId,
		tagIds: inserted.map((tag) => tag.id)
	});
};

function isSeedTagsBody(body: unknown): body is { count: number; organizationId?: string | null } {
